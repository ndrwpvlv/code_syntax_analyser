...
class Report(Filters):
    ...
    def __init__(self, data: list, filename: str, template_jinja: str):
        """
    ...
    def txt_write(self):
        template = Template(self.template)
        with open('{}.txt'.format(self.filename), 'w') as fw:
            fw.write(template.render(data=self.data))

    def json_write(self):
        with open('{}.json'.format(self.filename), 'w') as fw:
            fw.write(json.dumps(self.data))

    def csv_write(self):
        with open('{}.csv'.format(self.filename), 'w') as fw:
